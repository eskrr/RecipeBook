<?php
	...
	$consulta = "SELECT Recipe.id, Recipe.name, Recipe.created_at, Recipe.description, User.name AS 'author_name', AVG(Rating.value) AS rating FROM Recipe INNER JOIN User ON Recipe.author_id = User.id INNER JOIN Rating ON Rating.recipe_id = Recipe.id GROUP BY Recipe.id";
